# api/services/salesforce_service.py
import requests
import json
...
class SalesforceChatService:
    def __init__(self):
        # Passo 1: O "Dicionário de Dados" (Schema)
        # A 'description' é a chave para a IA entender o propósito de cada campo.
        self.SALESFORCE_SCHEMA = {
            ...
            # Adicione outros objetos aqui no futuro (Account, Lead, etc.)
        }
        self.termos_objetos = {
    ...
    def _plan_soql_query(self, pergunta: str) -> dict:
        ...
        response = openai_client.chat.completions.create(
            model="gpt-4-turbo",  # Usar um modelo mais recente é recomendado para JSON
    ...
    def _build_soql_from_plan(self, plan: dict) -> str:
        ...
        if filters:
            where_clauses = []
            for f in filters:
                # Sanitização simples para evitar problemas de formatação
    ...
    def _formatar_resposta(self, records: list, plan: dict) -> str:
        ...
        # Constrói o cabeçalho da resposta
        ...
        for i, record in enumerate(records):
            ...
            detalhes = []
            # Itera sobre os campos do registro para montar os detalhes
            for campo, valor in record.items():
                if campo.lower() != 'attributes':  # Ignora o campo 'attributes'
    ...
    def _process_query_sync(self, request: ChatRequest) -> dict:  # <-- Agora retorna um dicionário
        ...
        try:
            ...
            # Formata a resposta usando o método que já tínhamos
    ...
    async def query(self, request: ChatRequest) -> dict:  # <-- Já retorna um dicionário
