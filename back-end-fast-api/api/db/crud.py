...
# 👇 Ajuste a função de criação
def create_user(db: Session, user_claims: dict):
    groups = user_claims.get("groups", [])
    departamento = groups[0].lstrip('/') if groups else None

    db_user = models.Usuario(
        # id continua sendo gerado localmente
        keycloak_id=user_claims.get("sub"), # <-- Salva o ID do Keycloak
...
def create_chat(db: Session, usuario_id: str, chat: ChatCreate): # <-- Assinatura atualizada
    db_chat = models.Chat(
        id=uuid.uuid4(),
        usuario_id=usuario_id,      # <-- Usa o usuario_id passado como argumento
        titulo=chat.titulo
    )
    db.add(db_chat)
    db.commit()
